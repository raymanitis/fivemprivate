...
local function CreateLog(resource, data)
    ...
    if data.players then
        if type(data.players) == "table" then
            ...
            for _, v in pairs(data.players) do
                table.insert(fields, {
                    name = v.role .." Name & IDs",
                    value = GetPlayerName(v.id) .." ["..v.id.."] - `".. GetPlayerIdentifierByType(v.id, Config.MainIdentifier) .."`",
                    inline = false,
                })
            end
        end
            ...
            for i = 1, #data.info do
                if data.info[i].value then
                    if type(data.info[i].value) == "table" then
                        text = text .."\n**".. data.info[i].name .."**: `".. json.encode(data.info[i].value) .."`"
                    else
                        text = text .."\n**".. data.info[i].name .."**: ".. data.info[i].value ..""
                    end
                end
            end
        elseif type(data.info) == "string" then
            text = data.info
        end
    end

    -- Add invoking resource to the log
    if resource and not data.silentSource then
    ...
    -- Post log to Discord
    PerformHttpRequest(Config.Webhooks[data.category], function(err, text, headers) end, 'POST', json.encode({["username"] = Config.ServerName, ["avatar_url"] = Config.ServerLogo, embeds = {embed}, content = data.tagEveryone and "@everyone" or nil}),  { ['Content-Type'] = 'application/json' })
end
