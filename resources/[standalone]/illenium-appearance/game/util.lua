...
function ToFloat(num)
	assert(type(num) == "number", ("param `num` is not a number, num: (%s), type: (%s)"):format(num, type(num)))
	return num + 0.0
end
...
local function getPedHeadBlend(ped)
	...
	local shapeFirst, shapeSecond, shapeThird, skinFirst, skinSecond, skinThird, shapeMix, skinMix, thirdMix = Citizen
		.InvokeNative(0x2746BD9D88C5C5D0, ped,
...
local function setPedFaceFeatures(ped, faceFeatures)
	assert(type(ped) ~= "nil", "[setPedFaceFeatures] param `ped` is nil.")
	assert(faceFeatures, "[setPedFaceFeatures] param `faceFeatures` is nil")

	for i = 1, #constants.FACE_FEATURES do
		local v = constants.FACE_FEATURES[i]

		if not v or not faceFeatures[v] then
			goto continue
		end

		SetPedFaceFeature(ped, i - 1, ToFloat(faceFeatures[v]))
...
local function setPedHeadOverlays(ped, headOverlays)
	if not headOverlays then return log.errorf("[game] [util] [setPedHeadOverlays] param `headOverlays` is nil.") end

	for i = 1, #constants.HEAD_OVERLAYS do
		local v = constants.HEAD_OVERLAYS[i]
		local headOverlay = headOverlays[v]

		if not headOverlay or not headOverlay.opacity then
			goto continue
		end

		SetPedHeadOverlay(ped, i - 1, headOverlay.style, ToFloat(headOverlay.opacity))

		if not headOverlay.color then
			goto continue
		end
		...
		SetPedHeadOverlayColor(ped, i - 1, colorType, headOverlay.color, headOverlay.secondColor)
