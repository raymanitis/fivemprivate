local Scaleforms = require 'drill_minigame/config'
local Drilling = {}
...
Drilling.Start = function(callback)
    if not Drilling.Active then
        ...
        TaskPlayAnim(cache.ped, "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 8.0, -8.0, -1, 49, 0,
                false, false, false)
...
Drilling.Update = function(callback)
    while Drilling.Active do
        ...
        if IsControlJustPressed(0, 200) then
            ...
            ClearPedTasks(cache.ped) -- Clear the animation
            StopSound(soundId)
