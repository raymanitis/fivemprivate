...
window.addEventListener('message', function (event) {
    ...
    PlayerIsCrimeHost = event.data.status
        ...
        const message = DOMPurify.sanitize(event.data.message, {
            ALLOWED_TAGS: [],
            ALLOWED_ATTR: []
        })

        const chatMessage1 = $("<div>").addClass("chatMessage");
        const pfp1 = $("<div>").addClass("pfp").html('<i class="fa-solid fa-user-secret"></i>');
        const text1 = $("<div>").addClass("message").text(message);
        chatMessage1.append(pfp1, text1);

        const chatMessage2 = $("<div>").addClass("chatMessage");
        const pfp2 = $("<div>").addClass("pfp").html('<i class="fa-solid fa-user-secret"></i>');
        const text2 = $("<div>").addClass("message").text(message);
        chatMessage2.append(pfp2, text2);

        $(".chatMessages").append(chatMessage1);
