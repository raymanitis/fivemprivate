...
<div class="w-screen h-screen flex items-center justify-end { menuRight ? 'flex-row' : 'flex-row-reverse' } " transition:fly="{{ x: menuRight ? 400 : -400 }}">
  ...
  <div class="w-[3.2vh] h-[85%] flex flex-col gap-[1vh]" class:ml-[1vh]={!menuRight} class:mr-[1vh]={menuRight}>
    ...
    <!-- REFRESH ALERTS -->
    <button class="w-full h-[3vh] flex items-center justify-center bg-primary/80 backdrop-blur-sm hover:bg-secondary/80 text-text_primary"

      on:click={() => {
        SendNUI("refreshAlerts");
      }}
    >
      <i class="fas fa-arrows-rotate text-[1.5vh] text-text_primary"></i>
    </button>
    <!-- TOGGLE MUTE -->
    <button class="w-full h-[3vh] flex items-center justify-center bg-primary/80 backdrop-blur-sm hover:bg-secondary/80 text-text_primary"
      on:click={toggleMute}
    >
      <i class="fas fa-volume-{$DISPATCH_MUTED ? "xmark" : "high"} text-[1.5vh] text-text_primary"></i>
    </button>
    <!-- TOGGLE ALERTS -->
    <button class="w-full h-[3vh] flex items-center justify-center bg-primary/80 backdrop-blur-sm hover:bg-secondary/80 text-text_primary"
      on:click={toggleAlerts}
    >
      <i class="fas fa-{$DISPATCH_DISABLED ? "bell-slash" : "bell"} text-[1.5vh] text-text_primary"></i>
    </button>
    <!-- CLEAR BLIPS -->
    <button class="w-full h-[3vh] flex items-center justify-center bg-primary/80 backdrop-blur-sm hover:bg-secondary/80 text-text_primary"
      on:click={() => {
    ...
    >
    <i class="fas fa-ban text-[1.5vh] text-text_primary"></i>
    ...
    >
    <i class="fas fa-{menuRight ? "hand-point-left" : "hand-point-right"} text-[1.5vh] text-text_primary"></i>
  ...
  <div class="w-[25%] h-[97%] overflow-auto pr-[0.5vh]" class:ml-[2vh]={!menuRight} class:mr-[2vh]={menuRight}>
    ...
    <button class="w-full h-fit mb-[1vh] font-medium {dispatch.priority == 1 ? 'bg-priority_secondary/80 backdrop-blur-sm' : 'bg-secondary/80 backdrop-blur-sm'}" on:click={() => toggleDispatch(dispatch.id)}>
        <div class="flex items-center gap-[1vh] p-[1vh] text-[1.5vh] {dispatch.priority == 1 ? " bg-priority_primary/80 backdrop-blur-sm" : " bg-primary/80 backdrop-blur-sm"}">
            <p class="px-[2vh] py-[0.2vh] rounded-md bg-accent_green text-white font-semibold">
              #{dispatch.id}
            </p>
            <p class="px-[2vh] py-[0.2vh] rounded-md text-white font-semibold {dispatch.priority == 1 ? " bg-accent_red" : "bg-accent_cyan"}">
              {dispatch.code}
            </p>
            <p class="py-[0.2vh] text-text_primary">
              {dispatch.message}
            </p>
            <i class="{dispatch.icon} py-[0.2vh] ml-auto mr-[0.5vh] {dispatch.priority == 1 ? " text-accent_red" : "text-accent_cyan"}"></i>
          </div>
          <div class="flex flex-col p-[1vh] gap-y-[0.4vh] text-[1.4vh] w-full text-start text-text_primary">
        ...
        <div class=" mb-[1vh]" transition:slide={{ duration: 300 }}>
          {#if dispatch.units.length > 0}
            <div class="flex flex-col gap-[0.2vh] mb-[1vh] bg-primary/80 backdrop-blur-sm">
              {#each dispatch.units.slice(0, additionalUnitsVisible[dispatch.id] ? dispatch.units.length : 3) as unit}
                <div class="w-full h-[5vh] flex {dispatch.priority == 1 ? 'bg-priority_tertiary/80 backdrop-blur-sm' : 'bg-tertiary/80 backdrop-blur-sm'} flex items-center font-medium text-text_primary">
                  <p class="ml-[2vh] px-[1.4vh] py-[0.2vh] rounded-full {dispatch.priority == 1 ? 'bg-priority_secondary/80 backdrop-blur-sm' : 'bg-secondary/80 backdrop-blur-sm'}">{unit.metadata.callsign}</p>
                  <p class="mx-[1vh] px-[1.5vh] py-[0.2vh] rounded-full uppercase text-white {unit.job.type == "leo" ? "bg-[#004ca5] " : unit.job.type == "ems" ? "bg-[#e03535]" : "bg-[#4b4b4b]" }">{unit.job.name}</p>
              ...
              {#if dispatch.units.length > 3}
                {#if !additionalUnitsVisible[dispatch.id]}
                  <button class="w-full h-[5vh] flex items-center justify-center {dispatch.priority == 1 ? 'bg-priority_tertiary/80 backdrop-blur-sm' : 'bg-tertiary/80 backdrop-blur-sm'} flex items-center font-medium text-text_primary" on:click={() => toggleAdditionalUnits(dispatch.id)}>
          ...
          {/if}
          <button class="w-full h-[5vh] text-white font-semibold {dispatch.priority == 1 ? " bg-accent_red" : " bg-accent_green"} flex items-center font-medium"
            on:click={() => {
            ...
            <p class="mx-[2vh] px-[2vh] py-[0.2vh] rounded-full text-white {dispatch.priority == 1 ? " bg-accent_dark_red" : "  bg-accent_dark_green"} ">{dispatch.units.length} {$Locale.units}</p>
            <p class="ml-[3vh]">
              {#if CheckIfAttached(dispatch.units, $PLAYER.citizenid)}
