...
<div class="w-screen h-screen flex justify-end { $IS_RIGHT_MARGIN ? 'flex-row' : 'flex-row-reverse' } items-end">
  <div class="w-[25%] h-[97%]"
    ...
    {#each notifications.slice().reverse() as dispatch, index (dispatch.data.id)}
      <div class="w-full h-fit my-[0.5vh] font-medium {dispatch.data.priority == 1 ? " bg-priority_secondary/80 backdrop-blur-sm" : " bg-secondary/80 backdrop-blur-sm"}" transition:fly="{{ x: $IS_RIGHT_MARGIN ? 400 : -400 }}">
        <div class="flex items-center gap-[1vh] p-[1vh] text-[1.5vh] {dispatch.data.priority == 1 ? " bg-priority_primary/80 backdrop-blur-sm" : " bg-primary/80 backdrop-blur-sm"}">
          <p class="px-[2vh] py-[0.2vh] rounded-md bg-accent_green text-white font-semibold">
            #{dispatch.data.id}
          </p>
          <p class="px-[2vh] py-[0.2vh] rounded-md text-white font-semibold {dispatch.data.priority == 1 ? " bg-accent_red" : "bg-accent_cyan"}">
            {dispatch.data.code}
          </p>
        ...
        <div class="flex">
          <div class="flex flex-col p-[1vh] gap-y-[0.4vh] text-[1.4vh] w-[70%] text-text_primary">
              {#if dispatch.data}
          ...
          <div class="w-[30%] flex items-end justify-center mb-[1vh]">
            {#if index === 0}
              <p class="px-[1.5vh] py-[0.4vh] rounded-full text-[1.3vh] text-text_primary {dispatch.data.priority == 1 ? " bg-priority_primary/80 backdrop-blur-sm" : " bg-primary/80 backdrop-blur-sm"}">
